...
Pixify Pro هو النسخة الاحترافية من أداة تحويل الصور. يدعم **جميع الصيغ الحديثة** مع إمكانية التحكم الكامل في الجودة والإعدادات.
...
# توليد رمز API (مطلوب: الترخيص + اسم مستعار؛ اختياري: مدة الانتهاء بالأيام)
pixify-pro --generate-token --license-key PIXIFY-PRO-XXXX-XXXX-XXXX --token-alias "GitHub Actions"
# أو مع انتهاء بعد 365 يوماً:
# pixify-pro --generate-token --license-key PIXIFY-PRO-XXXX-XXXX-XXXX --token-alias "CI" --token-expiry 365
```

**ملاحظة:** الرمز الكامل يُعاد من الخادم **مرة واحدة فقط** عند التوليد — انسخه واحفظه في مكان آمن (مثلاً متغير بيئة أو أمانات CI). الرموز تُخزَّن على الخادم بشكل آمن (هاش)، وفي لوحة التحكم تظهر بشكل مُقنَّع (مثل pix_****abcd). يمكن إلغاء الرمز من لوحة التحكم باستخدام معرف الرمز والترخيص.
